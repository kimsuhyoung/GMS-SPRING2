...
<div style="height: 40px;width:300px;"></div>
	...
	<table id="member_list_tab">
		...
		<tr>
			...
			<td>${i.id}</td>
			<td><a id="list_name" onclick="app.controller.detailStudent('${i.id}')">${i.name}</a></td>
			<td>${i.ssn}</td>
			<td>${i.phone}</td>
			<td>${i.email}</td>
			<td>${i.title}</td>
			<td>${i.regdate}</td>
			<td>
			<a id="list-update-Btn" onclick="app.controller.updateStudent('${i.id}')">수정</a>
			/
			<a id="list-delete-Btn" onclick="app.controller.deleteTarget('${i.id}')">삭제</a>
			</td>
		</tr>
		</c:forEach>
	</table>
	  ...
	  <ul class="pagination">
	    ...
	    <c:forEach varStatus="i" begin="${startPage}" 
	    	...
	    	<c:choose>
	    		...
	    		<c:otherwise>
	    			<li id="pageNumber"><a href="#" onclick="app.member.list('${i.index}')">
	    				${i.index}</a></li>
	    		</c:otherwise>
	    	</c:choose>
	    </c:forEach>
