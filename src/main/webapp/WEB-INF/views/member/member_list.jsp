...
<div style="height: 40px;width:300px;"></div>
	<table id="member_list_tab">
		...
		<tr>
			<td>${i.num}</td>
			<td>${i.id}</td>
			<td><a id="list_name" onclick="detailStudent('${i.id}')">${i.name}</a></td>
			<td>${i.ssn}</td>
			<td>${i.phone}</td>
			<td>${i.email}</td>
			<td>${i.title}</td>
			<td>${i.regdate}</td>
			<td>
			<a id="list-update-Btn" onclick="updateStudent('${i.id}')">수정</a>
			/
			<a id="list-delete-Btn" onclick="deleteStudent('${i.id}')">삭제</a>
			</td>
		</tr>
		</c:forEach>
	</table>
